...
def backtest_start():
    """Start a backtest session in a background thread."""
    global predictor
    ...
    if len(df) < params['lookback'] + params['pred_len']:
        return jsonify({
            'error': f'Insufficient data: need at least {params["lookback"] + params["pred_len"]} rows, got {len(df)}'
        }), 400

    # Clean up old sessions (>1 hour)
    now = datetime.datetime.now()
    expired = [
        sid for sid, sess in backtest_sessions.items()
        if hasattr(sess, 'created_at') and (now - sess.created_at).total_seconds() > 3600
    ]
    for sid in expired:
        del backtest_sessions[sid]

    session_id = str(uuid.uuid4())
    session = BacktestSession(df, predictor, params)
